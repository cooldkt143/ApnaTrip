...
function ItineraryViewCard({ trip, index, onDelete, onUpdate, onViewItinerary }) {
  const [isEditing, setIsEditing] = useState(false);
  const [editedTrip, setEditedTrip] = useState({ ...trip });

  const handleDelete = (e) => {
    e.stopPropagation();
    onDelete?.(editedTrip.id);
  };
  ...
  const togglePinned = (e) => {
    e.stopPropagation();
    const updatedTrip = { ...editedTrip, pinned: !editedTrip.pinned };
    setEditedTrip(updatedTrip);
    if (!isEditing) onUpdate?.(updatedTrip);
  };

  const handleViewItinerary = (e) => {
    e.stopPropagation();
    onViewItinerary?.(editedTrip);
  };
    ...
    >
      ...
      >
        ...
        <div className="relative overflow-hidden">
          ...
          {/* Pin */}
          <button onClick={togglePinned} className="absolute top-4 right-4 z-10">
            <Star
        ...
        <div className="p-4 text-white">
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {[editedTrip.status, editedTrip.effort, editedTrip.type].map((tag, i) => (
              <span
                key={i}
          ...
          {/* Actions */}
          <div className="flex justify-between items-center gap-3 text-base opacity-80 group-hover:opacity-100 transition">
            {isEditing ? (
              <FaSave onClick={handleSave} className="cursor-pointer text-[#f5d07e]" />
            ) : (
              <>
                <FaEdit
                  onClick={(e) => {
            ...
            <button
              ...
              className="bg-[#f5d07e] text-black px-3 py-1 rounded hover:bg-yellow-300 transition"
            >
              View Itinerary
            </button>
          </div>
...
export default ItineraryViewCard;
